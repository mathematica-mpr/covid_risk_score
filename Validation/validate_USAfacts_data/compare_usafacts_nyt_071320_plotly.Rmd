...
format_names <- function(dat){
  n <- names(dat)
  n <- gsub("_", " ", n)
  n <- gsub("^(.)","\\U\\1", tolower(n), perl = TRUE)
...
# {.tabset}
There are several national-level aggregations of county-level COVID case and death data.

Each source has its own data collection methods, which can result in variations in numbers across sources.
...
## 1) Comparison of total national cases and deaths over time {.tabset}
```{r}
...
bind_rows(nyt_total, usafacts_total) %>% select(-deaths) %>% spread(source, cases) %>% 
...
### Deaths
```{r, fig.height=8, fig.width=10}
bind_rows(nyt_total, usafacts_total) %>% select(-cases) %>% spread(source, deaths) %>% 
  filter(date>=date("2020-04-01")) %>%
  mutate(difference = NYT-USAFacts,
         percent_difference = 100*(NYT-USAFacts)/USAFacts) %>%
...
## 2) Comparison of unallocated cases
```{r}
...
g <- bind_rows(nyt_unknown, usafacts_unknown) %>% filter(cases>0) %>%
  ...
  rename(State=state, `Number unallocated`=val, Source=source)  %>%
  
  ggplot(aes(x=State, y=`Number unallocated`, fill=Source)) +
...
## 3) Comparison of counties with reported cases in the two data sets
...
## 4) Comparison of cumulative cases for all counties that appear in both data sets
```{r}
g <- bind_rows(dat_nyt %>% select(date, fips, cases) %>% mutate(source="NYT"),
  ...
  ggplot(aes(x=NYT, y=USAFacts, text=County)) + geom_point(size=.5) +
...
g <- case_diff_tab %>% 
  ...
  ggplot(aes(x=NYT, y=USAFacts, text=County)) + 
...
g %>% ggplotly(tooltip = c("text","x", "y")) %>% 
  layout(xaxis = list(range = c(0,27800)), yaxis = list(range = c(0,27800)))
```
...
## 5) Comparison of cumulative deaths for all counties that appear in both data sets
```{r}
