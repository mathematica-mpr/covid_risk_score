---
title: "Create and validate priors for de-noising of county-level CFR"
date: "8/25/2020"
...
dat <- readRDS("usafacts_082420.RDS") %>% 
  gather(var,count,c("cases","deaths")) %>%
  filter(var=="deaths" & date == max(date) | var=="cases" & date == max(date)-13) %>%
  select(-date) %>% spread(var, count) %>%
  filter(cases>0) %>%
...
#      seed=090120) 
#  
# fit1 %>% saveRDS(file.path("fit1.RDS"))

fit1 <- readRDS(file.path("fit1.RDS"))
```
...
adjust_CFR_EB <- function(deaths, cases, shape1, shape2, ...){
  (deaths + shape1)/(cases+shape1+shape2)
}
...
```{r}
#state_fitted_dists %>% dplyr::select(state, shape1, shape2) %>%
#  write_csv("state_CFR_priors.csv")
```
